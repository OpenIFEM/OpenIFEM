...
namespace IFEM
{
  ...
  {
    this->detF = dealii::determinant(F);
    const dealii::Tensor<2, dim> FBar = Physics::Elasticity::Kinematics::F_iso(F);
    this->bbar = dealii::Physics::Elasticity::Kinematics::b(FBar);
    Assert(this->detF > 0, ExcInternalError());
  }
  ...
  dealii::SymmetricTensor<4, dim> HyperelasticMaterial<dim>::getJcIso() const
  {
    ...
    const dealii::SymmetricTensor<4, dim> ccBar = getCcBar(); // fictitious elasticity tensor
    return (2.0/dim)*dealii::trace(tau_bar)*ST::dev_P
      - (2.0/dim)*(tau_iso_x_I + I_x_tau_iso) + ST::dev_P*ccBar*ST::dev_P;
  }
