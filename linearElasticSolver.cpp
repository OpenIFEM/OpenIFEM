...
namespace
{
  ...
  /**
   * Helper function that returns the contribution to strain epsilon(i, j) at
   * the q-th quadrature point made by the a-th shape function where a is in the
   * range of dofs_per_cell. Note that shape_grad_component(a, q, i) returns the
   * full gradient of the i-th component of the a-th shape function at the q-th
   * quadrature point... Reference:
   * http://www.dealii.org/8.5.0/doxygen/deal.II/step_18.html
   */
  template <int dim>
        ...
        for (unsigned int j = i; j < dim; ++j)
          {
            strain[i][j] = (feValues.shape_grad_component(a, q, i)[j] +
                            feValues.shape_grad_component(a, q, j)[i]) /
                           2;
          }
      }
    return strain;
   ...
   * FEValues::get_function_gradients extracts the gradients of each component
   * of the solution field at a quadrature point, which is [u_{i, j}, v_{i, j},
   * w_{i, j}]. We simply take this vector to compute the strain in a
   * straight-forward way.
   */
  template <int dim>
  inline SymmetricTensor<2, dim>
  getStrain(const std::vector<Tensor<1, dim>> &grad)
  {
...
namespace IFEM
{
  ...
  template <int dim>
  void LinearElasticSolver<dim>::localAssemble(
    const typename DoFHandler<dim>::active_cell_iterator &cell,
    AssemblyScratchData &scratch,
    AssemblyCopyData &data)
  {
    auto mat =
      std::dynamic_pointer_cast<LinearElasticMaterial<dim>>(this->material);
            ...
            for (unsigned int q = 0; q < numQuadPts; ++q)
              {
                data.cell_matrix(i, j) +=
                  getStrain(scratch.fe_values, i, q) * elasticity *
    ...
    for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; ++i)
      {
        if (cell->face(i)->at_boundary() &&
            this->bc.traction.find(cell->face(i)->boundary_id()) !=
              this->bc.traction.end())
          {
            scratch.fe_face_values.reinit(cell, i);
            Tensor<1, dim> traction =
              this->bc.traction[cell->face(i)->boundary_id()];
                ...
                for (unsigned int j = 0; j < dofsPerCell; ++j)
                  {
                    ...
                    data.cell_rhs(j) +=
                      ...
                      traction[component_j] * scratch.fe_face_values.JxW(q);
                  }
              }
          }
      }
    ...
    for (unsigned int i = 0; i < data.dof_indices.size(); ++i)
      {
        for (unsigned int j = 0; j < data.dof_indices.size(); ++j)
          {
            this->tangentStiffness.add(
              data.dof_indices[i], data.dof_indices[j], data.cell_matrix(i, j));
          }
  ...
  void LinearElasticSolver<dim>::globalAssemble()
  {
     ...
     */
    WorkStream::run(
      this->dofHandler.begin_active(),
      this->dofHandler.end(),
      *this,
      &LinearElasticSolver::localAssemble,
      &LinearElasticSolver::scatter,
  ...
  void LinearElasticSolver<dim>::output(const unsigned int cycle) const
  {
    ...
    std::string fileName =
      "LinearElastic-" + Utilities::int_to_string(cycle, 4) + ".vtu";
    std::ofstream out(fileName.c_str());
    // Pitfall! StrainPostprocessor must be declared before DataOut because the
    // latter is holding a pointer to the former!
    ...
    data.add_data_vector(this->dofHandler,
                         this->solution,
  ...
  void LinearElasticSolver<dim>::runDynamics(const std::string &fileName)
  {
    ...
    Vector<double> temp1(
      this->dofHandler.n_dofs()); // temp1 = un + dt*vn + dt^2*(0.5-beta)*an
    Vector<double> temp2(this->dofHandler.n_dofs()); // temp2 = -K*temp1 + Fn+1

    // a0 = F0/M
    MatrixCreator::create_mass_matrix(this->dofHandler,
                                      this->quadFormula,
                                      this->mass); // rho=1
