#ifndef MPI_SUPG_SOLVER
#define MPI_SUPG_SOLVER
...
namespace Fluid
{
  namespace MPI
  {
    ...
    /*! \brief the parallel SUPG Navier Stokes equation solver base class
     *
     ...
     *
     * SUPG fluid solvers share the same preconditioner and solving process,
     * which are implemented in this base class. Only the assembly method is
     * implemented in the actual SUPG type solvers.
     *
     ...
     * does modify the linear system a little bit.
     */
    template <int dim>
    ...
    public:
      //! Constructor.
      SUPGFluidSolver(parallel::distributed::Triangulation<dim> &,
    ...
    protected:
       ...
       * reassembled at every Newton iteration. The Dirichlet BCs are applied at
       * the same time as the cell matrix and rhs are distributed to the global
       * matrix and rhs, which is optimal according to the deal.II
       * documentation. The boolean argument is used to determine whether
       * nonzero constraints or zero constraints should be used.
       * This is a pure virtual method in SUPG solver. It is implemented in
       * derived classes that are the actual fluid solvers.
       */
      virtual void assemble(const bool use_nonzero_constraints) = 0;
