function [value,A] = detA_hp(lambda,k,d,p)
...
flds = fieldnames(p);
for j = 1:numel(flds)
    p.(flds{j}) = vpa(p.(flds{j}));
end


beta_f = sqrt(k^2 + lambda*p.rho_f/p.mu);        if real(beta_f)<0, beta_f = -beta_f; end
gamma  = sqrt(k^2 + p.rho_s*lambda.^2/p.mu_s);   if real(gamma )<0, gamma  = -gamma ; end
chi    = sqrt(k^2 + p.rho_s*lambda.^2/(p.lambda_s+2*p.mu_s));
if real(chi)<0, chi = -chi; end

cg   = cosh(gamma*d);  sg   = sinh(gamma*d);
cchi = cosh(chi*d);    schi = sinh(chi*d);
