  ...
  if (params.dimension == 3)
    {
      ...
      dealii::GridGenerator::subdivided_hyper_rectangle(
        ...
        Point<3>(-0.68, -0.68, 0),
        Point<3>(0.68, 0.68, 0.272),
        true);
      // Create Lagrangian solid object with hyper elastic material
      Solid::LinearElasticity<3> solid(lag_tria, params);
      // Constrain all the points in both directions
      std::vector<Point<3>> points;
      std::vector<unsigned int> directions;
      std::vector<bool> vertex_touched(lag_tria.n_vertices(), false);
      for (auto cell = lag_tria.begin_active(); cell != lag_tria.end();
         ++cell)
       {
