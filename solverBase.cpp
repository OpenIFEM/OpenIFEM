...
namespace IFEM
{
  ...
  void SolverBase<2>::generateMesh()
  {
    GridGenerator::subdivided_hyper_rectangle(
      this->tria,
      std::vector<unsigned int>{128, 16},
      Point<2>(0., 0.),
      Point<2>(8., 1.),
      true);
    std::cout << "Mesh info: " << endl
              << "dimension: 2"
              << " number of cells: " << this->tria.n_active_cells()
              << std::endl;
  }
  ...
  void SolverBase<3>::generateMesh()
  {
    GridGenerator::subdivided_hyper_rectangle(
      this->tria,
      std::vector<unsigned int>{32, 4, 4},
      Point<3>(0., 0., 0.),
      Point<3>(8., 1., 1.),
      true);
    std::cout << "Mesh info: " << endl
  ...
  void SolverBase<dim>::readBC(const std::string &fileName)
  {
        ...
        if (type == "traction" || type == "gravity")
          {
            ...
            if (components.size() < static_cast<unsigned int>(dim))
              {
                throw runtime_error(
                  "Traction/gravity has insufficient number of components!");
              }
            Tensor<1, dim> temp;
        ...
        else if (type == "displacement")
          {
            ...
            if (dofs.size() != vals.size())
              {
                throw runtime_error(
                  "Displacement bc has different numbers of dofs and values!");
              }
    ...
    cout << endl;
    for (auto itr = this->bc.traction.begin(); itr != this->bc.traction.end();
         ++itr)
      {
    ...
    for (auto itr = this->bc.pressure.begin(); itr != this->bc.pressure.end();
         ++itr)
      {
        cout << "boundary_id = " << itr->first << " pressure = " << itr->second
             << endl;
      }
    for (auto itr = this->bc.displacement.begin();
         itr != this->bc.displacement.end();
         ++itr)
      {
  ...
  template <int dim>
  void SolverBase<dim>::applyBC(SparseMatrix<double> &A,
                                Vector<double> &x,
                                Vector<double> &b)
  {
    ...
    for (auto itr = this->bc.displacement.begin();
         ...
         ++itr)
      {
        ...
        Functions::ConstantFunction<dim> function(
          itr->second.second); // value of the BC
        ComponentMask mask(itr->second.first);
        VectorTools::interpolate_boundary_values(
          this->dofHandler, id, function, boundaryValues, mask);
      }
