  ...
  for (auto cell = shell_solver.triangulation.begin_active();
       ...
       ++cell)
    {
      ...
      for (unsigned int i = 0; i < unit_points.size(); ++i)
        {
          p[i]->interpolator =
            std::make_shared<Utils::GridInterpolator<3, Vector<double>>>(
      ...
      for (unsigned int i = 0; i < unit_points.size(); ++i)
        {
          ...
          p[i]->interpolator->point_value(solid_solver.current_displacement,
                                          d_value);
          p[i]->interpolator->point_value(solid_solver.current_velocity,
                                          v_value);
